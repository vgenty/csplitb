...
class CSplitB(object):
    ...
    def __init__(self, spliton, infile, number, prefix, suffix, events):
    ...
    def write(self, data):
        ...
        if self.events < 0: return
        if self.count >= self.events: sys.exit(1)
...
def main(argv = sys.argv):
    ...
    parser.add_argument("-e", "--events", type=int, help="Number of events you want. Default is -1")
    args = parser.parse_args(argv[1:])

    csplitb = CSplitB(args.spliton, args.infile, args.number, args.prefix, args.suffix, args.events)
    return csplitb.run()
